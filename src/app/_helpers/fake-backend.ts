...
export class FakeBackendInterceptor implements HttpInterceptor {
    ...
    intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
        ...
        return of(null).pipe(mergeMap(() => {
            ...
            // get student by Id
            if (request.url.match(/\/student\/\d+$/) && request.method === 'GET') {
              // check for fake auth token in header and return user if valid, this security is implemented server side in a real application
              if (request.headers.get('Authorization') === 'Bearer fake-jwt-token') {
                  // find user by id in users array
                  let urlParts = request.url.split('/');
            ...
            // validation
            let duplicateStudent = students.filter(student => {
              if(student.rollNumber === newStudent.rollNumber) {
                dupeStudentName = student.studentName;
              }
              return student.rollNumber === newStudent.rollNumber;
            }).length;
            if (duplicateStudent) {
              console.log('Dupes');
                return throwError({ error: { message: 'Roll Number "' + newStudent.rollNumber + '" is already assigned to another student "' + dupeStudentName + '"' } });
            }

            // save new student
            newStudent.id = newStudent.rollNumber;
            students.push(newStudent);
            localStorage.setItem('students', JSON.stringify(students));

            console.log('Student Saved: ' + JSON.stringify(newStudent));
            // respond 200 OK
            return of(new HttpResponse({ status: 200 }));
            }
